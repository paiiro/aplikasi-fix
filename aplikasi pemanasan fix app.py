...
if halaman == "Beranda":
    st.title("🍽️ Reaksi Bahan Pangan Terhadap Suhu")
    ...
    st.title("🔥 Reaksi Bahan Pangan Terhadap Suhu")

    mode = st.radio("🎭 Simulasi Sebagai:", ("Chef", "Analis"))
    makanan = st.selectbox("🔍 Pilih Bahan Pangan:", list(data_makanan.keys()))

    suhu_target = st.slider("🌡️ Atur Suhu Target (°C):", 0, 300, 70)
    durasi = st.slider("⏱️ Durasi Simulasi (menit):", 1, 10, 3)

    suhu_akhir = 0
    tampilkan_reaksi = []

    if st.button("🚀 Jalankan Simulasi"):
        total_iterasi = durasi * 10
        suhu_akhir = suhu_target
        progress = st.progress(0)

        for i in range(total_iterasi + 1):
            suhu_now = int(i / total_iterasi * suhu_target)
            tampilkan_reaksi = get_multi_reaksi(makanan, suhu_now, mode)
            progress.progress(i / total_iterasi)
            time.sleep(0.01)

        suhu = suhu_target
        st.success(f"✅ Simulasi selesai! Suhu akhir: {suhu}°C")
